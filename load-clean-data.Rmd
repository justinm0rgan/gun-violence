...
if(!require(tidycensus)) install.packages("tidycensus", repos = 
...
```
### Census data and US states shapefile

Get census data with tidycensus package to get ratio of population to gun violence for aggregation by state map.

```{r census-data}
# load variables for decennial 
dec_var <- tidycensus::load_variables(2020,"pl",cache = T)

# download population and state geometry
df_state_pop <- tidycensus::get_decennial(geography = "state",
                          year = 2020,
                          geometry = TRUE,
...
```{r load-states-shapefile}
# url
url_state <- "https://www2.census.gov/geo/tiger/TIGER2017//STATE/tl_2017_us_state.zip"
...
download.file(url_state, tf)
# get the name of the shp file in the zip archive
fname = unzip(tf, list = TRUE)$Name[4]
# unzip file to the temp dir
unzip(tf, files = fname, exdir = td, overwrite = TRUE)
# fpath is the full path to the extracted file
fpath = file.path(td, fname)

(us_states <- st_read(fpath, drivers = "shp"))
```
```{r upload-from-data-folder}
(us_states <- st_read("./data/shp/tl_2017_us_state/tl_2017_us_state.shp"))
```
...
Plot each record within date span for US.

```{r plot-lat-lon-within-date}
all_addresses <- all_gun_violence_full$address_state
full_sf <- geocode_OSM(all_addresses,
            keep.unfound = T,
            details = T,
            as.sf = T,
            geometry = "point")
