...
all_gun_violence_full <- all_gun_violence_abbr %>% 
  mutate(
    address = str_remove(address, " block of"),
    address = str_remove(address, "Block of"),
    address = gsub(".*(and{1})","",address),
    address_full = paste(address, city_or_county, abbreviation,
                              sep = ", "))
all_gun_violence_full <- all_gun_violence_abbr %>% 
  mutate(
    county_state = paste(city_or_county, state,
                         sep = ", ")
  )

all_gun_violence_full
```
